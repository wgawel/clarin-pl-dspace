...
public class OAIIndexEventConsumer implements Consumer
{
    ...
    OAIIndexingService indexer = dspace.getServiceManager().getServiceByName(
            OAIIndexingService.class.getName(), OAIIndexingService.class);
    ...
    public void consume(Context ctx, Event event) throws Exception
    {
        ...
        if (subject != null)
        {
            if (event.getSubjectType() == Constants.COLLECTION
                    || event.getSubjectType() == Constants.COMMUNITY)
            {
                if (et == Event.MODIFY || et == Event.MODIFY_METADATA
                        || et == Event.REMOVE || et == Event.DELETE)
                {
                    // must update all the items
                    if (subject.getType() == Constants.COMMUNITY)
                    {
                        for (Collection col : ((Community) subject)
                                .getCollections())
                        {
                            addAll(col);
                        }
                    ...
                    || event.getSubjectType() == Constants.BUNDLE)
            {
                ...
                {
                    itemsToUpdate.addAll(Arrays.asList(((Bundle) subject)
                            .getItems()));
                }
            }
    ...
    public void end(Context ctx) throws Exception
    {
        ...
        try
        {
            if (itemsToUpdate != null)
            {
                ...
                indexer.indexItems(ctx, filtered);
            }
        }
        catch (Exception e)
        {
