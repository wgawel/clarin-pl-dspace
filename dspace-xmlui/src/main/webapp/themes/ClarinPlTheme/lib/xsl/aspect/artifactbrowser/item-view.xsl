		...
		<div id="files_section">			
			<h4>
				...
				<xsl:if test="/mets:METS/@OBJID">							
					...
					<dl id="process-progres" class="dl-horizontal small" style="display:none">
						<dt>Processing files:</dt>
						<dd>
							<div class="progress right">
								<div id="p-progress" class="progress-bar" role="progressbar" data-transitiongoal="75" aria-valuemin="0" aria-valuemax="100"></div>
							</div>
						</dd>
					</dl>
...
<!-- 				</xsl:otherwise>
	...
	<xsl:template name="export-to-inforex">
		...
		<a id="export-to-inforex" class="label label-info" style="display:none;">
			...
			<i18n:translate>
				<i18n:text>xmlui.UFAL.artifactbrowser.item-export-to-inforex</i18n:text>
			</i18n:translate>
		</a>
	</xsl:template>

	<xsl:template name="goto-freq-lists">
		<xsl:param name="goto-freq-lists" />
		<xsl:variable name="freqListURL"
			select="confman:getProperty('dspace.freqention.list.url')" />
		<a id="goto-freq-lists" class="label label-info" style="display:none;">
			<xsl:attribute name="href">javascript:{}</xsl:attribute>
			<xsl:attribute name="onclick">document.getElementById('freq_form').submit(); return false;</xsl:attribute>
			<i class="fa fa-exchange">&#160;</i>
			<i18n:translate>
				<i18n:text>xmlui.UFAL.artifactbrowser.item-goto-freq-lists</i18n:text>
			</i18n:translate>
		</a>
		<form id="freq_form" style="display:none;" target="_newtab">
			<xsl:attribute name="action"><xsl:value-of select="$freqListURL" /></xsl:attribute>
			<xsl:attribute name="method">POST</xsl:attribute>
			<input id="freq_token" type="hidden" name="token" />
		</form>
	</xsl:template>
