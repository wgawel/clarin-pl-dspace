		...
		<xsl:if
			...
			<span id="file_upload">
				<div id="file_upload_container" style="display:none">
					...
					<![CDATA[
						var clarinpl_upload_file_alert_max_file_size = ]]><xsl:value-of select="$lr.upload.file.alert.max.file.size" /><![CDATA[; 						
						var clarinpl_help_mail = "]]><xsl:value-of select="$lr.help.mail" /><![CDATA[";
					]]>
					</script>
                ...
                <div>
                        ...
                        <xsl:for-each select="dri:field/dri:option[not(@returnValue='')]">
	                        ...
	                        <img style="width: 64px; height: 64px">
	                                <xsl:attribute name="src"><xsl:copy-of select="$theme-path" />/../ClarinPlHomeTheme/lib/images/<xsl:value-of select="@returnValue" />.png</xsl:attribute>
	                        </img>
