...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="buildHead">
		<head>
			...
			<link rel="stylesheet" href="{$theme-path}/lib/clarinpl/public/css/clarinpl.css" />
            ...
            <link rel="stylesheet" href="{$theme-path}/lib/bootstrap/css/clarinpl-theme.css" />
	...
	<xsl:template match="dri:body">
		...
		<div class="container-fluid">
			<div class="container">
				...
				<!-- Top banner with CLARIN-PL logo -->
	...
	<xsl:template match="dim:dim" mode="recentList-DIM-metadata">
		...
		<img class="artifact-icon pull-right" alt="{dim:field[@element='type'][1]/node()}">
			<xsl:attribute name="src">
                                <xsl:text>themes/ClarinPlHomeTheme/lib/images/</xsl:text>
                                <xsl:value-of
	...
	<xsl:template name="addJavascript">
		...
		<script type="text/javascript">
			<xsl:attribute name="src">
	             <xsl:value-of select="$theme-path" />
	             <xsl:text>/lib/bootstrap/js/clarinpl.min.js</xsl:text>
			</xsl:attribute>&#160;</script>
